...
class CreateJobSeekerForm(d_forms.ModelForm):
    password = d_forms.CharField(label='Password', widget=d_forms.PasswordInput)
    password2 = d_forms.CharField(label='Confirm Password', widget=d_forms.PasswordInput)
    ...
    def __init__(self, *args, **kwargs):
        super(CreateJobSeekerForm, self).__init__(*args, **kwargs)
        for field_name, field in self.fields.items():
            field.widget.attrs.update({
                'class': 'bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-indigo-600 focus:border-indigo-600 block w-full p-2.5',
                'placeholder': field.label,
        ...
        self.fields['username'].required = True
        self.fields['email'].required = True
        self.fields['primary_skills'].required = True
        self.fields['higher_qualification'].required = True
        self.fields['password'].required = True
    ...
    def clean(self):
        ...
        cleaned_username =  cleaned_data.get('username',None)
        if cleaned_email is not None and cleaned_username is not None:
            employer_qs = Employer.objects.filter(Q(email=cleaned_email) | Q(username=cleaned_username))
            if employer_qs.exists():
                raise d_forms.ValidationError(_("Email or Username is already registered with another employer."))
        ...
        if cleaned_email:
            ...
            if not (re.fullmatch(email_reg, cleaned_email)):
                raise d_forms.ValidationError("Please enter valid email id. (E.g: youremail@example.com)")
    ...
    class Meta:
        model = JobSeeker
        fields = ("first_name", "last_name", "email", 
                  "username", "primary_skills", "higher_qualification", "password")
